...
class Improvements extends Component {
  constructor(props) {
    ...
    if (!cards[this.state.activeSlide].done) {
      var dontSet =
        cards[this.state.activeSlide].willDo ||
        cards[this.state.activeSlide].done;
      ...
      this.setState(
        (prevState, props) => ({
          ...
          animate: true,
          potentialScore:
            this.state.potentialScore + (dontSet ? 0 : this.state.unit)
        }),
        function() {
          this.next();
        }
  ...
  clear() {
    ...
    if (!cards[this.state.activeSlide].clear) {
      var dontSet =
        cards[this.state.activeSlide].clear ||
        (!cards[this.state.activeSlide].willDo &&
          !cards[this.state.activeSlide].done);
      cards[this.state.activeSlide].done = false;
      cards[this.state.activeSlide].willDo = false;
    ...
    return (
      ...
      >
        <div className="comfortscore-top comfortscore-activated">
          <h2>
            <strong>Forslag</strong> til {this.state.address}
          </h2>
        </div>
          ...
          <div className="comfortscore-twocol">
            ...
            <div className="comfortscore-col">
              <div className="comfortscore-instruction">
                <TextRow
                  text={
                    "Følgende løsninger kan forbedre komforten hjemme hos dig. Vælg dem, du er interesseret i at gå videre med og se, hvordan det forbedrer din komfortscore. Husk: Du kan altid gå tilbage og justere dine valg."
                  }
                />
              </div>
        ...
        <div className="comfortscore-action">
          <button
            className="comfortscore-btn comfortscore-btn-back"
            onClick={this.goBack}
          >
            Tilbage
          </button>
          <p className="comfortscore-label-btn">
            Se din liste med forbedringstiltag og hvordan du kan gemme den til
            senere
          </p>
          <button
            className="comfortscore-btn comfortscore-btn-success"
            data-src="{action: 'load', eventLabel: 'improvements', noninteractive: false}"
